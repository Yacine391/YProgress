...
import {SafeAreaView,View,Text,StyleSheet,Pressable,ScrollView,TextInput,Alert,Switch,Image,Modal} from "react-native";
...
import {WEEK_DAYS,WORKOUTS,dayByKey,dayKeyForDate} from "./src/features/training/program";
...
const FALLBACK_VIDEO="https://videos.pexels.com/video-files/4761426/4761426-hd_1920_1080_25fps.mp4";
function ExerciseVideoModal({exercise,onClose}){
  const player=useVideoPlayer(exercise?.video||FALLBACK_VIDEO,p=>{p.loop=true});
  useEffect(()=>{if(exercise)player.play();else player.pause()},[exercise,player]);
  return <Modal visible={Boolean(exercise)} transparent animationType="fade" onRequestClose={onClose}>
    <View style={S.modalBackdrop}><View style={S.videoModal}>
      ...
      <VideoView player={player} style={S.video} nativeControls contentFit="cover"/>
...
function ProgramPage({selectedDayKey,setSelectedDayKey,week,setWeek,drafts,onSaveSet,history,onComplete,hardDay,onEmergency,progressionContext}){
  ...
  return <ScrollView style={S.pageScroll} contentContainerStyle={S.content} keyboardShouldPersistTaps="handled">
    ...
    {workout.type==="REST"?<Card style={S.restDay}><Text style={S.restTitle}>RÉCUPÉRATION</Text><Text style={S.coach}>Pas de séance prévue aujourd’hui.</Text>{workout.priorities.map(item=><View key={item} style={S.priorityRow}><Text style={S.check}>✓</Text><Text style={S.text}>{item}</Text></View>)}<Text style={S.restFoot}>Aucune charge n’est calculée pendant un jour REST.</Text></Card>:<>
      ...
      {workout.exercises.map((exercise,index)=>{
        const draftKey=`${week}:${exercise.id}`,sets=drafts[draftKey]?.sets||[];
        const exerciseHistory=historyForExercise(history,exercise.id);
        const recommendation=strength?recommendNextLoad({exercise,history:exerciseHistory,context:progressionContext}):null;
        return <Card key={exercise.id} style={S.exerciseCard}>
          <View style={S.exerciseTop}><View style={S.exerciseNumber}><Text style={S.exerciseNumberText}>{index+1}</Text></View><View style={{flex:1}}><Text style={S.exerciseName}>{exercise.name}</Text><Text style={S.muted}>{exercise.muscles}</Text></View><Pressable onPress={()=>setVideoExercise(exercise)} style={S.videoButton}><Text style={S.videoButtonText}>▶ Voir exo</Text></Pressable></View>
          {strength?<><View style={S.prescriptionRow}><Text style={S.prescription}>{exercise.sets} × {exercise.minReps}–{exercise.maxReps}</Text><View style={S.loadChip}><Text style={S.loadChipLabel}>{recommendation.action==="start"?"DÉPART":recommendation.action==="increase"?"↑ PROGRESSION":recommendation.action==="decrease"?"↓ ADAPTÉE":"→ MAINTIEN"}</Text><Text style={S.loadChipValue}>{recommendation.recommendedLoad} kg</Text></View></View><Text style={S.reason}>{recommendation.reasons[0]}</Text>
            {Array.from({length:exercise.sets},(_,setIndex)=>{const set=sets[setIndex]||{};return <View key={setIndex} style={S.setRow}><Text style={S.setLabel}>S{setIndex+1}</Text><Field style={S.setField} label="KG" value={String(set.load??"")} onChange={v=>onSaveSet(exercise,setIndex,"load",v)} keyboardType="decimal-pad"/><Field style={S.setField} label="REPS" value={String(set.reps??"")} onChange={v=>onSaveSet(exercise,setIndex,"reps",v)} keyboardType="numeric"/><Field style={S.setField} label="RIR" value={String(set.rir??"")} onChange={v=>onSaveSet(exercise,setIndex,"rir",v)} keyboardType="numeric"/></View>})}
            <Button title="Valider la performance" onPress={()=>onComplete(exercise,recommendation)} secondary/>
          ...
          </>:<><Text style={S.rest}>Durée : {exercise.duration} • Repos : {exercise.rest}</Text><Text style={S.reason}>Cette activité influence la récupération, jamais une charge arbitraire.</Text></>}
        </Card>})}
